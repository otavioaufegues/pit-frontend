...
export default function ReportDepartment({route, navigation}) {
  const {year, departmentId} = route.params;
  ...
  const categoryReportHandler = data => {
    const dataResults = data.map(item => {
      return {
  ...
  const axisReportRadarHandler = (departmentData, allDepartmentData) => {
    ...
    let allDepartmentTeaching = 0;
    let allDepartmentSearch = 0;
    let allDepartmentExtension = 0;
    let allDepartmentAdministrativeActivity = 0;
    let allDepartmentTrainingAndRepresentation = 0;
    const dataResults = [];
    ...
    const allDepartment = allDepartmentData.map(item => item._id.department);
    const allDepartmentUnique = allDepartment.reduce((arr, el) => arr.concat(arr.includes(el) ? [] : [el]), []);
    allDepartmentData.map(item => {
      if (item._id.axis[0].name == 'Ensino') allDepartmentTeaching += item.totalActivitiesByAxis;
      if (item._id.axis[0].name == 'Pesquisa') allDepartmentSearch += item.totalActivitiesByAxis;
      if (item._id.axis[0].name == 'Extensão') allDepartmentExtension += item.totalActivitiesByAxis;
      if (item._id.axis[0].name == 'Atividade Administrativa') allDepartmentAdministrativeActivity += item.totalActivitiesByAxis;
      if (item._id.axis[0].name == 'Capacitação e Representação') allDepartmentTrainingAndRepresentation += item.totalActivitiesByAxis;
    });

    // console.log('allDepartmentUnique', allDepartmentUnique.length);
    // console.log('trainingAndRepresentation', trainingAndRepresentation);
    // console.log('allDepartmentTrainingAndRepresentation', allDepartmentTrainingAndRepresentation);
    ...
    dataResults.push(
      ...
      {
        Ensino: allDepartmentTeaching / allDepartmentUnique.length || 0,
        Pesquisa: allDepartmentSearch / allDepartmentUnique.length || 0,
        Extensão: allDepartmentExtension / allDepartmentUnique.length || 0,
        AtividadeAdministrativa: allDepartmentAdministrativeActivity / allDepartmentUnique.length || 0,
        CapacitaçãoRepresentação: allDepartmentTrainingAndRepresentation / allDepartmentUnique.length || 0,
      },
    );
  ...
  const getActivitiesCountByCategory = async (year, departmentId) => {
    try {
      const {data} = await api.getActivitiesCountByCategoryByDepartmentService(year, departmentId);
  ...
  const getActivitiesCountByAxis = async (year, departmentId) => {
    try {
      const uniqueDepartment = await api.getActivitiesCountByAxisByDepartmentService(year, departmentId);
      const allDepartment = await api.getActivitiesCountByAxisAllDepartmentService(year);
      setActivitiesCountByAxis(uniqueDepartment.data.totalActivitiesByAxis);
      axisReportHandler(uniqueDepartment.data.totalActivitiesByAxis);
      axisReportRadarHandler(uniqueDepartment.data.totalActivitiesByAxis, allDepartment.data.totalActivitiesByAxis);
      setLoading(false);
    } catch (e) {
      console.log(e);
    }
  ...
  useEffect(() => {
    setLoading(true);
    getActivitiesCountByCategory(year, departmentId);
    getActivitiesCountByAxis(year, departmentId);
  }, [route]);
        ...
        {activitiesCountByAxis == undefined || dataReportsByAxis == undefined || dataRadar == undefined || maximaRadar == undefined ? (
          ...
          <View style={styles.alignVertical}>
            ...
            <VictoryChart polar theme={VictoryTheme.material} domain={{y: [0, 1]}}>
              <VictoryGroup colorScale={['tomato', 'orange']} style={{data: {fillOpacity: 0.2, strokeWidth: 2}}}>
            ...
            <VictoryLegend
              ...
              gutter={20}
              data={[
                {name: 'Departamento', symbol: {fill: 'orange'}},
                {name: 'Instituição', symbol: {fill: 'tomato'}},
              ]}
            />
          </View>
        )}
